...
pygame.display.set_caption('Conduction for Metals')
...
for column in range(11): #along x
    y2=147
    for row in range(6): # along y
        angle2 = random.uniform(0,math.pi*2)
        particle = Particle((x2, y2), e_size, e_density*e_size**2)
        particle.colour = (250, 0, 0)
        particle.speed = originalspeed
        angle2+=math.pi/2
        particle.angle = angle2
        my_particles.append(particle)
        y2+=25
    x2+=30
...
while running:
    ...
    for i in range(len(my_particles)):
        if i < rows*columns:
            my_particles[i].move()
            my_particles[i].bounce(originalposition[i][1]-7, originalposition[i][1]+7, originalposition[i][0]-7, originalposition[i][0]+7)
        else:
            my_particles[i].move()
            my_particles[i].bounce(140, 150 + boxheight, 160, 170 + boxwidth)
    ...
    if button < 2:
        for row in range(rows):
            my_particles[row].speed = 0.02
        for elect in range(rows*columns+1,len(my_particles)):
            my_particles[elect].speed = 0.2
    if button == 2:
        for row in range(rows):
        ...
        for elect in range(rows*columns+1,len(my_particles)):
            if my_particles[elect].speed > 2:
                my_particles[elect].speed = 2
    if button == 3:
        for row in range(rows):
            my_particles[row].speed = 1
    ...
    if button > 7:
        ...
        for elect in range(rows*columns+1,len(my_particles)):
            if my_particles[elect].speed > 2:
                my_particles[elect].speed = 2
    #pygame.draw.circle(screen, (150, 200, 0), (174+20, 167-20), 5, 60)# blue
    #pygame.draw.circle(screen, (200, 100, 200), (245, 180), 20, 20) # pink
    #pygame.draw.circle(screen, (100, 200, 0), (245, 225), 20, 20) #lime green
    if button > 0:
